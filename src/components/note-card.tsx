...
export function NoteCard({note}:NoteCard){
        return(
            <Dialog.Root>
            <Dialog.Trigger  className='rounded-md text-left flex flex-col bg-slate-800 p-5 gap-y-3 overflow-hidden  relative outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus:ring-lime-400'>
            ...
            <Dialog.Portal>    
                <Dialog.Overlay className='inset-0 fixed bg-black/50'/>
                 <Dialog.Content className='fixed overflow-hidden left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 max-w-[640px] h-[60vh] w-full bg-slate-700 rounded-md flex flex-col outline-none'>
                    ...
                    > 
                    Deseja <span className='text-red-400 group-hover:underline hover:text-slate-100'>apagar essa nota ?</span>
                    </button>
                    </Dialog.Content>
            </Dialog.Portal>
